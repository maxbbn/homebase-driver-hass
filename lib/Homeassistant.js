...
/**
 * user Auth 兼容 password 和 oauth2 token
...
function Homeassistant(userAuth) {
  // long live token 和 oauth 2.0 都使用 token 方式验权
  this.authType = userAuth.ext3 === TYPE_PASSOWRD ? TYPE_PASSOWRD : TYPE_OAUTH2
  this.hassURL = userAuth.ext1 || userId
  this.password = userAuth.userToken
...
Homeassistant.prototype._request = function (method, path, body, options) {
  return new Promise((resolve, reject) => {
    ...
    if (this.authType === TYPE_PASSOWRD) {
      options.headers['x-ha-access'] = this.password;
    } else if (this.authType === TYPE_OAUTH2) {
